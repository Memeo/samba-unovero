...
public:
	...
	MagniLinks(Json::Value p_links) :
			_admin(p_links[admin].asString()), _home(p_links[home].asString()), _logout(
					p_links[logout].asString()) {
	}
...
public:
	MagniAuthResponse(Json::Value jvalue) :
			_magni_links(jvalue["links"]) {
	}
...
int main(int argc, const char * argv[]) {
	...
	bool show_headers;
	options.add_options()("help,h", "produce help message")("headers,H",
			"print headers")("source,s", po::value<std::string>(&source),
			"source URL");
	...
	{
		std::cout << request.protocol() << "://" << request.host() << ":" << request.port() << request.uri().path()  << "?" << request.query() << std::endl;
	...
	request.uri("https://magni-staging.memeo.com/thor/magni/files");
	request << ::boost::network::header("Memeo-Magni-Auth-Token", jvalue["magni_token"].asString());
	{
		std::cout << request.protocol() << "://" << request.host() << ":" << request.port() << request.uri().path()  << "?" << request.query() << std::endl;
		headers_range<http_client::response>::type headers_ =
				request.headers();
	...
	memberNames = jvalue.getMemberNames();
